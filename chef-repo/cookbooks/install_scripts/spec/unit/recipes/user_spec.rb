  ...
  ['14.04', '16.04'].each do |version|
    context "When all attributes are default, on an Ubuntu #{version} platform" do
      ...
      it 'does not create a user' do
        expect(chef_run).to_not create_user('user').with(
            home: '/home/user',
            shell: '/bin/bash',
...
describe 'install_scripts::user' do
  ['14.04', '16.04'].each do |version|
    context "When user name and password are specified, on an Ubuntu #{version} platform" do
      let(:chef_run) do
        ChefSpec::SoloRunner.new(platform: 'ubuntu', version: version) do |node|
          node.set['install_scripts']['user']['name'] = 'user'
          node.set['install_scripts']['user']['password'] = '$6$YPpcwnaZtxEObrRR$RpqD1RgLVHOrr0BbAmVocf.6/yIU5.mW.hr.sm./xvwheoa9xBO6td71PUczQKfcaAcE1U2c3o0mOMb8ufvr5/'
